...
!
! !REVISION HISTORY:
!  23 Apr 2023: Shahryar Ahmad; Initial Specification
!
! !INTERFACE:
  subroutine read_SSURGO_mukey(n, num_bins, mukeygrd,  mukey )
! !USES:
    use LDT_coreMod
    use LDT_coreMod
    use LDT_logMod
...
! !ARGUMENTS: 
    ...
    real,    intent(out)  :: mukeygrd(LDT_rc%lnc(n),LDT_rc%lnr(n),num_bins)
    real,    intent(out)  :: mukey(LDT_rc%lnc(n),LDT_rc%lnr(n),num_bins)

! !DESCRIPTION:
!  This subroutine retrieves SSURGO mukey data 
...
!  \item[mukeygrd]
!   output field with mukey grid
!  \item[mukey]
!   output field with mukey 
!  \end{description}
!EOP
    integer :: c,r
    logical :: file_exists
!________________________________________________________________________
    ...
    write(LDT_logunit,*) "[INFO] Reading SSURGO mukey file: ",&
         trim(LDT_rc%mufile(n))  


   mukey  = 0.
   mukeygrd = 0.    
! -------------------------------------------------------------------
!  CHECK FOR AND OPEN MUKEY FILE
! -------------------------------------------------------------------
!- Sand file:
    inquire(file=trim(LDT_rc%mufile(n)), exist=file_exists)
    if(.not.file_exists) then 
       write(LDT_logunit,*) "SSURGO mukey file ",trim(LDT_rc%mufile(n))," not found."
       write(LDT_logunit,*) "Program stopping ..."
       call LDT_endrun
    endif

    call processSSURGOmukey(n, LDT_rc%mufile(n), num_bins, mukey)

    ! Make sure and negative values are set to universal undefined value:
    do r=1,LDT_rc%lnr(n)
       do c=1,LDT_rc%lnc(n)
          if( mukey(c,r,1) < 0. ) mukey(c,r,1) = LDT_rc%udef
       enddo
    enddo
    mukeygrd(:,:,1) = 1.0

    write(LDT_logunit,*) "[INFO] Done reading SSURGO mukey file."
    
  end subroutine read_SSURGO_mukey

!BOP
! !ROUTINE: processSSURGOmukey
! 
! !DESCRIPTION:
!  This subroutine reads a given SSURGO mukey 
!  file and processes the data to 
!  the LDT target grid. 
!
! !INTERFACE: 
  subroutine processSSURGOmukey(n, filename, num_bins, output_field)
! !USES:
    use LDT_coreMod
...
!EOP  
    ...
    integer                 :: subpnc, subpnr, glpnc, glpnr 	! Total number of input param grid array points
    integer                 :: mi     							! Total number of output LIS grid array points
    integer                 :: mo     							! Input parameter grid desc array
    real                    :: subparam_gridDesc(20)         
    integer, allocatable    :: lat_line(:,:), lon_line(:,:)		! Maps each input grid point to output grid.
    integer, allocatable    :: n11(:)  							! input parameter 1d grid
    real,    allocatable    :: gi1(:), gi2(:)  					! input logical mask (to match gi)
    ...
    integer               :: x_offset, y_offset					! Read input parameter
    real,    allocatable  :: read_inputparm(:,:)   				! input parameter 1d grid
    real,    allocatable  :: gi(:)     							! input logical mask (to match gi)
    logical*1,allocatable :: li(:)    							! output lis 1d grid
    real      :: go1(LDT_rc%lnc(n)*LDT_rc%lnr(n),numtiles)  	! output logical mask (to match go1)
...
!     stop
   ...
   end subroutine processSSURGOmukey
